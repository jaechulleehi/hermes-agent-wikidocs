...
이 책은 추상적인 AI 에이전트 개념서가 아니다. 공식 Hermes Agent 기능을 실제 업무 운영 맥락으로 번역하고, OpenClaw에서 Hermes로 이사하며 겪은 시행착오를 체크리스트와 판단 기준으로 바꾸는 실전 운영서다.
...
## 1-1. 최근 보강 소스: OpenClaw 커뮤니티 대화 CSV
...
| API/OAuth/구독제 비용과 provider 설정 | 0장 provider/model/config |
| Slack/gateway 봇 멈춤과 delivery 문제 | 5장 gateway/cron, 9장 권한 |
| Memory.md 장기화, Obsidian/Notion/RAG 연결 | 4장 기억 시스템 |
| 공개 Skill 활용과 내부 Skill 변환 | 6장 Skill 운영 |
| 커뮤니티/팀 채널의 멀티봇 trigger 규칙 | 8장 멀티봇 규칙, 11장 실제 운영 사례 |

공유 본문에는 개인 닉네임, 원문 대화, 내부 채널 정보, 보호해야 할 계정/토큰/경로를 넣지 않는다. 대신 질문 유형과 판단 기준만 추출한다.
...
0장 기초 가이드는 공식 문서와 검색 초입 질문을 연결하는 관문이다. 공식 GitHub/공식 Docs/Installation/Quickstart/OpenClaw migration/Docker/Gateway/Claude Code/Codex 비교와 Slack/Telegram/Discord 같은 메시징 채널 질문은 0장에서 먼저 받고, 1장 이후는 AI 개인비서와 역할형 에이전트 운영 구조로 이어진다.
...
| Personality/SOUL.md | 4장 | SOUL.md와 `/personality`로 에이전트 정체성/톤을 정의 | 하비/뽀동이 같은 역할형 에이전트의 실제 답변 차이와 AGENTS.md/memory 분리 기준으로 설명 | `04-10-soul-md-agent-personality.md` 반영 |
...
| Mac mini 상시 Hermes 운영, tmux/launchd/절전/gateway 분리 | 0장/5장/9장 | 전용 머신에서 AI 개인비서를 계속 켜둘 때 필요한 운영 기준 | 1차 반영 |
...
| 크론 조사에서 WikiDocs 발행까지 이어지는 콘텐츠 운영 흐름 | 11장 | 자동화/역할 분리/발행 기준이 한 번에 보이는 대표 케이스 | 1차 반영 |
| 방울이 확장조사와 뽀동이 글쓰기 handoff | 11장 | 조사형과 정리형 에이전트 협업 기준을 보여주는 케이스 | 1차 반영 |
| GitHub와 WikiDocs로 발행하고 수정하는 흐름 | 11장/7장 | GitHub source of truth와 WikiDocs 공개 채널 기준을 보여주는 케이스 | 1차 반영 |
| Slack 스레드에서 하비가 일을 분배하는 방식 | 11장/8장 | 메인 창구, 호출 규칙, 역할 분리 기준이 드러나는 케이스 | 1차 반영 |
| SEO/GEO 치트시트를 콘텐츠 자산으로 만드는 흐름 | 11장/7장 | 이미지 제작, QA, skill화, 배포 문안, WikiDocs 기록이 이어지는 콘텐츠 자산화 케이스 | 1차 반영 |
| 반복 문서 제작을 템플릿/데이터/QA 흐름으로 줄이는 케이스 | 11장/6장 | 수료증 같은 개인별 산출물을 샘플 확정, 명단 적용, PNG/ZIP 생성, 누락 검수로 처리하는 반복 업무 자동화 케이스 | 1차 반영 |
...
| 5장 | 도구/자동화 | 공식 docs 기능을 붙일 때 권한, gateway, cron, delivery, launchd/절전에서 어디가 막혔는가? |
| 6장 | Skill 운영 | 반복 업무가 언제 Skill이 되고, stale Skill은 어떻게 고쳐야 하는가? |
| 7장 | 콘텐츠 시스템 | 좋은 조사 결과가 글로 약해진 사례, 위키가 자산이 된 사례는 무엇인가? |
| 8장 | 운영 FAQ | 멀티봇 스레드가 시끄러워진 실제 상황과 호출 규칙은 무엇인가? |
| 9장 | 보안/복구/마이그레이션 | OpenClaw→Hermes 이사 중 실제로 체크해야 했던 순서와 보안 경계는 무엇인가? |
| 10장 | 조직 확장 | 조직 도입에서 모델보다 운영 병목이 먼저 드러난 사례는 무엇인가? |
| 11장 | 실제 운영 케이스 | 크론/Slack/GitHub/WikiDocs와 역할형 에이전트가 한 흐름으로 묶인 실제 사례는 무엇인가? |
...
- 체크리스트나 판단 기준으로 끝나는가?
- 하비/방울이/뽀동이 같은 내부 이름보다 역할 개념이 먼저 나오는가?
- 민감한 토큰, webhook, 개인 경로, 계정 정보가 제거되어 있는가?
- 다음 글로 이어지는 연결 문장이 있는가?
- 실제 운영 케이스라면 발행 기준, 보호 정보 제거 기준, 재사용 체크리스트가 들어가는가?

## 10. 4장 기억 시스템 개편 메모

- 4장은 `AI 에이전트 기억 시스템은 어떻게 설계해야 할까`로 확장했다.
- Obsidian LLM Wiki는 7장 콘텐츠 시스템이 아니라 4장 외부 장기 기억층으로 이동했다.
...
4. 11장 실제 운영 케이스를 크론 조사, handoff, GitHub/WikiDocs 발행, Slack 분배 흐름 중심으로 보강한다.
5. 9장에 OpenClaw→Hermes 이사 방법과 복구 체크리스트를 실제 절차로 보강한다.
6. 이후 7장 콘텐츠 시스템과 10장 조직 확장을 확장한다.
