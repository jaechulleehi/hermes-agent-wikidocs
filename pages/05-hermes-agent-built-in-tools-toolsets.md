...
### 도구가 실패하면 모델을 바꿔야 하나요?

먼저 도구 권한, 실행 환경, 경로, 인증, 로그를 확인한다. 많은 실패는 모델 문제가 아니라 연결/권한/검증 문제다.

## 다음 글

다음에는 Hermes Agent terminal 도구와 terminal backend를 언제 쓰고 언제 조심해야 하는지 본다.
