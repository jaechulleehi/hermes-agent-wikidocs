...
에르메스 에이전트(Hermes Agent)로 공개 글과 운영 문서를 만들 때 조사형과 정리형은 모두 품질에 영향을 준다. 하지만 실패 방식은 다르다. 조사형이 실패하면 근거가 약해지고, 정리형이 실패하면 흐름은 있어 보이지만 내용이 비어 보인다. 이 차이는 [조사형 에이전트](https://wikidocs.net/345895)와 [정리형 에이전트](https://wikidocs.net/345896)를 따로 운영해야 하는 이유다.
...
| 실제감이 없음 | 실제 사례를 찾았는가 | 사례를 독자용 이야기로 바꿨는가 |
| 공개 범위가 애매함 | 원자료에서 제외할 세부값을 표시했는가 | 독자에게 필요한 이야기만 남겼는가 |

즉 조사형은 재료의 품질을 책임지고, 정리형은 읽히는 구조를 책임진다. 둘 중 하나만 좋아서는 공개 글의 품질이 안정되지 않는다.

## 실제 예시: 링크 문제와 본문 보강

WikiDocs 작업에서 처음 내부 링크를 보강했을 때, 단순히 “이어서 읽기”를 붙이는 방식으로는 부족했다. 독자는 문장을 읽다가 개념이 궁금해지는 순간에 이동해야 한다. 이 피드백은 조사형과 정리형의 역할이 함께 필요한 사례다.
...
메인 창구가 기준을 정해야 한다. 조사형의 근거가 더 필요할 수도 있고, 정리형이 독자 흐름에 맞게 재배열해야 할 수도 있다.

### 어떤 역할이 더 중요하나요?

둘 다 중요하다. 조사형은 재료를 만들고, 정리형은 그 재료를 독자가 이해할 수 있는 흐름으로 바꾼다.
