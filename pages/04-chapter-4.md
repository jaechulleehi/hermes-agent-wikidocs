...
| WikiDocs | 공개 가능한 지식으로 재작성한 결과물 | 비공개 내부값과 독자가 볼 필요 없는 운영 흔적 |
...
그래서 4장은 단순한 기능 설명이 아니라, 실제 운영 중 생긴 질문을 장 구조로 바꾼 사례다. 요청 하나가 들어오면 하비가 기억 위치를 판단하고, 뽀동이가 공개 문서로 다시 구조화한다. 이 과정에서 내부 운영값은 덜어내고, 독자에게 필요한 기준만 남긴다.
...
뽀동이가 독자용 구조로 재작성
  ↓
GitHub/WikiDocs에 공개 가능한 지식으로 발행
```
...
5. 공개 문서로 바꿀 때 비공개 내부값과 독자가 볼 필요 없는 운영 흔적을 제거했는가?
