...
Hermes Agent를 실제 업무에 붙여보면 기억은 하나의 저장소가 아니다. 지금 대화의 session, 장기 선호를 담는 memory, 역할과 권한을 나누는 profile/AGENTS.md, 팀 공통 기준을 담는 shared-memory, 누적 지식 원장인 Obsidian LLM Wiki, 그리고 OpenViking/RAG 같은 외부 회수층이 함께 움직인다.

이 장은 AI 에이전트에게 더 많은 내용을 외우게 하는 법이 아니라, 기억을 층으로 나누고 하비 같은 메인 창구가 그 기억층을 어떻게 관리해야 하는지 설명한다.
...
| WikiDocs | 공개 가능한 지식으로 재작성한 결과물 | 내부 경로/토큰/비공개 운영값 |
...
5. 공개 문서로 바꿀 때 내부 경로, 토큰, 계정, 채널 ID를 제거했는가?
