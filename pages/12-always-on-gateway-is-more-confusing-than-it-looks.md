...
always-on gateway는 [AI 개인비서](https://wikidocs.net/345923)를 상시 대기 상태로 만드는 장치처럼 보인다. 하지만 실제 운영에서는 “켜져 있다”와 “제대로 응답할 준비가 됐다”가 다르다. 그래서 gateway는 생각보다 자주 헷갈린다.

[Hermes Agent](https://wikidocs.net/346055)에서 gateway를 다룰 때는 status 한 줄만 믿으면 안 된다. 어떤 profile이 떠 있는지, 실제 프로세스가 살아 있는지, autostart가 예전 설정을 물고 있지 않은지, 메시징 플랫폼과 연결이 살아 있는지까지 함께 봐야 한다.
...
heartbeat는 좋은 신호지만, 항상 충분한 증거는 아니다. heartbeat가 찍혀도 실제 메시지 전달이 막혀 있을 수 있고, status가 좋아 보여도 예전 [profile](https://wikidocs.net/345899)이 떠 있을 수 있다.

그래서 운영에서는 “상태 출력”과 “실제 동작”을 함께 확인해야 한다. 예를 들어 [Slack](https://wikidocs.net/345995)에서 응답이 필요한 구조라면 gateway status뿐 아니라 실제 Slack thread 응답, 로그, 프로세스, config를 같이 본다.
...
[다음 글: Daily Briefing Bot은 어떤 업무 자동화 패턴일까](https://wikidocs.net/345926)
