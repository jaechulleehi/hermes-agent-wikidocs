...
| 11-7 | 반복 문서 제작 자동화 | 샘플 템플릿 → 사람 피드백 → 명단 적용 → PNG 일괄 생성 → ZIP/QA 보고 |
| 11-8 | 메일 정리와 답장 초안 | 메일 분류 → 할 일 추출 → 답장 초안 → 사람 승인 → 보관/발송 |
| 11-9 | 자료조사와 결정 메모 | 질문 좁히기 → 근거 수집 → 비교표 → 리스크 정리 → 다음 액션 결정 |
...
|---|---|---|
| 매일 쌓이는 메일을 줄이고 싶다 | 11-8 메일 정리와 답장 초안 | 자동 발송보다 초안 생성/사람 승인을 먼저 둔다 |
| 새 주제를 빠르게 파악하고 싶다 | 11-9 자료조사와 결정 메모 | 링크 목록이 아니라 비교 기준과 다음 액션을 남긴다 |
| 매일/매주 반복되는 확인 업무를 줄이고 싶다 | 11-10 모닝 브리핑과 주간 리포트 | cron prompt와 전달 채널을 self-contained하게 만든다 |
| 블로그/WikiDocs/강의 자료를 만들고 싶다 | 11-2, 11-3, 11-4 | 조사/정리/발행/검증을 한 번에 묶지 않는다 |
| 이미지나 카드뉴스가 필요하다 | 11-1, 11-6 | 메시지 압축과 이미지 QA 기준을 먼저 정한다 |
| 같은 문서를 여러 번 만들어야 한다 | 11-7 반복 문서 제작 | 샘플 1개를 먼저 검수한 뒤 일괄 생성한다 |
