...
“자동화해줘”라고 말했는데, 결과가 더 불안해질 때가 있다.

![Hermes Agent AI 업무 자동화 사례 흐름](../assets/images/body-figures/ch11-overview-ai-workflow-cases-codex-only.webp)

메일을 자동으로 보내도 되는지, 조사 결과를 어디까지 믿어도 되는지, 이미지가 실제 글의 메시지와 맞는지, WikiDocs에 반영됐다는 말이 GitHub 커밋인지 공개 페이지 확인인지 헷갈린다. AI 업무 자동화가 어려운 이유는 도구가 부족해서가 아니라, **요청 → 역할 분리 → 검증 → 기록**의 순서가 없기 때문이다.
...
11장의 사례는 Hermes Agent를 단순 코딩 에이전트나 개인 심부름 도구로만 보지 않는 관점에서 정리했다. Claude Code/Codex가 코드 작업을 깊게 파고들고, OpenClaw가 개인 컴퓨터에서 일하는 AI 집사의 체감을 쉽게 보여준다면, 이 장의 Hermes 사례는 Slack 요청, 메일, 자료조사, 이미지 제작, WikiDocs 발행, cron 오류 복구가 어떻게 하나의 AI 업무 자동화 운영 흐름으로 이어지는지 보여준다.

특히 11장의 사례는 결과물만 보여주기 위한 장이 아니다. 메일 정리 기준이 Skill이 되고, 이미지 검수 기준이 다음 제작의 QA가 되고, 크론 오류가 복구 플레이북과 memory 기준으로 바뀌는 **자기 개선 루프**를 보여주기 위한 장이다. Hermes의 사례가 단순 활용법보다 운영 사례에 가까운 이유도 여기에 있다.
...
21개 사례를 번호순으로 읽을 필요는 없다. 로이드가 Slack에서 실제로 요청했던 흐름을 기준으로 보면 더 쉽다. 독자는 “어떤 에이전트를 쓸까?”보다 “내가 지금 줄이고 싶은 일이 무엇인가?”에서 출발하면 된다.
...
여기서 중요한 것은 Slack 대화를 그대로 복사하지 않는 것이다. Slack은 시작점이고, 책이나 팀 문서는 읽는 사람이 따라 할 수 있는 절차로 다시 정리해야 한다.

## 전체 사례 색인

| 묶음 | 사례 | 언제 보면 좋은가 |
|---|---|---|
| 이메일/고객응대 | 11-08, 11-16, 11-17, 11-21 | 메일, 고객 문의, 답장 초안, 승인 흐름, 외부 미팅 팔로업을 만들 때 |
| 조사/회의자료 | 11-09, 11-10, 11-14, 11-19, 11-21 | 리서치, 브리핑, 회의록, 의사결정 자료, SDR 파이프라인이 필요할 때 |
...
| 제품/실행/기억 | 11-05, 11-11, 11-12, 11-13, 11-22 | Slack 요청을 제품 요구사항, 실행 작업, 장기 기억, 개인 업무 OS로 바꿀 때 |
| 비개발자 업무 OS | 11-07, 11-13, 11-22, A-6 | 코딩 문법보다 업무 구조화와 검증 흐름부터 만들고 싶을 때 |
...
처음에는 Slack과 GitHub/WikiDocs만 있어도 충분하다. 이후 반복 주제가 생기면 cron을 붙이고, 반복 절차가 생기면 [Skill](https://wikidocs.net/346235)로 분리하면 된다. 비개발자라면 앱을 먼저 만들기보다 `내가 반복해서 하는 업무 → 필요한 입력 → 중간 확인 → 최종 산출물 → 다음에 남길 기준`을 먼저 적어보는 것이 좋다. 이 흐름이 보이면 바이브코딩 도구나 Claude Code/Codex 같은 코딩 에이전트도 더 안정적으로 쓸 수 있다.
