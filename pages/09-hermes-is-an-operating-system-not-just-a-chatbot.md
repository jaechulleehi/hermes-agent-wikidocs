...
에르메스 에이전트([Hermes Agent](https://wikidocs.net/346055))는 처음 보면 챗봇처럼 보인다. 질문을 받고 답하고, 도구를 쓰고, 메모를 남기니까 그렇다. 하지만 실제 업무에 붙여보면 중요한 지점은 답변 한 번의 품질이 아니다. 요청을 어떻게 해석하고, 누가 조사하고, 누가 정리하고, 어떤 프로필과 기억을 쓰고, 무엇을 검증하고, 무엇을 기록으로 남길지가 더 중요해진다.

그래서 Hermes는 단순 챗봇이라기보다 **AI 개인비서와 [역할형 에이전트](https://wikidocs.net/345925)가 함께 움직이는 운영 시스템**에 가깝다.
...
여기서 Hermes는 대화 상대가 아니라 운영 흐름의 접착제가 된다. Slack 요청을 받고, 파일을 확인하고, 세션 기록을 찾고, 본문을 고치고, 검증하고, GitHub에 남긴다. 답변만 잘하는 시스템이라면 이 흐름을 끝까지 책임지기 어렵다.
...
- shared-memory와 WikiDocs: 팀이 함께 보는 운영 기준과 공개 자산
...
실제 운영 시스템을 설명하려면 사례가 필요하다. 하지만 사례를 넣는다는 말은 내부 정보를 그대로 공개한다는 뜻이 아니다.
...
- 토큰, API 키, webhook, credential 값
- 개인 경로와 계정명
- 내부 채널 ID나 독자가 몰라도 되는 설정값
- 고객사나 개인이 특정될 수 있는 비공개 정보
...
### 원칙 6. 공개할 이야기와 숨길 정보를 분리한다

운영 판단은 공개 자산으로 만들고, 민감정보와 불필요한 내부값은 제거한다.
