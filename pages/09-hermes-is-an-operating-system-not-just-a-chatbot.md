...
![Hermes Agent는 단순 챗봇이 아니라 운영 시스템이다](../assets/images/body-figures/ch01-4-hermes-operating-system-body-codex-only.webp)
...
여기서 흔히 생기는 오해가 있다. AI 업무 자동화를 `딸깍`으로 이해하는 것이다. 버튼 한 번, 프롬프트 한 줄, 완성품 하나를 기대하면 처음 데모는 그럴듯해도 실제 업무에서는 금방 멈춘다. Hermes를 운영 시스템으로 봐야 하는 이유도 여기에 있다. 중요한 것은 한 번에 완성하는 능력이 아니라, 사람이 문제를 정의하고, 에이전트가 실행하고, 결과를 검증하고, 다음에 다시 쓸 기준으로 남기는 반복 구조다.

## 대화창은 입구이고, 운영 루프가 본체다

Hermes를 처음 보면 Slack이나 Telegram에서 대화하는 봇처럼 보일 수 있다. 하지만 메시지 플랫폼은 사용자가 Hermes와 만나는 문이다. 문 뒤에는 profile, memory, session search, tool, MCP, Skill, cron, gateway가 연결된 작업 흐름이 있다.

그래서 Hermes를 설명할 때 첫 질문은 `어떤 모델을 쓰는가`보다 `이 시스템이 내 작업 기준을 얼마나 오래 기억하고, 반복 업무를 얼마나 좋은 절차로 바꾸는가`에 가까워야 한다. 대화는 입구이고, 도구는 손이며, memory와 Skill은 시간이 지날수록 작업 품질을 안정시키는 운영층이다.
...
- shared-memory와 WikiDocs: 팀이 함께 보는 기준과 공유 자산
...
- 이 내용은 WikiDocs에 남길 이야기인가, 내부 기록에만 둘 정보인가?
...
Hermes에는 cron과 webhook처럼 사용자가 직접 말하지 않아도 실행되는 기능이 있다. 하지만 자동화는 처음부터 켜는 기능이 아니라, 수동으로 여러 번 성공한 흐름을 승격하는 단계다.
...
→ 입력과 산출물을 확인한다
→ 검증 기준을 만든다
→ 반복되는 절차를 Skill이나 체크리스트로 남긴다
→ 그다음 cron/webhook 자동화 후보로 올린다
```

이 순서를 지키면 자동화는 위험한 반복 실행이 아니라, 검증된 운영 절차의 반복 실행이 된다.

## 운영 시스템으로 볼 때의 기준

운영 시스템 관점에서는 기능을 많이 켜는 것보다 흐름을 끊기지 않게 만드는 것이 먼저다.

| 운영 층 | 확인 질문 | 흔한 실패 |
|---|---|---|
| 창구 | 사용자는 어디에 요청하는가 | CLI/Slack/cron이 따로 놀아 요청이 흩어진다 |
| 역할 | 누가 조사/정리/실행/검증을 맡는가 | 한 에이전트가 다 하다가 근거와 실행이 섞인다 |
| 상태 | 어떤 profile/session/memory를 보는가 | 같은 이름의 에이전트가 다른 기억으로 답한다 |
| 도구 | 실제로 무엇을 읽고 실행했는가 | 설명만 하고 파일/링크/화면 확인이 빠진다 |
| 기록 | 결과를 어디에 남기는가 | 대화 안에서만 끝나 다음에 다시 찾지 못한다 |
...
운영 판단은 공유 자산으로 만들고, 보호해야 할 정보와 덜어낼 세부값은 제거한다.
