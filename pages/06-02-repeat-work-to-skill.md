...
하망이가 WikiDocs 이미지를 만들 때도 반복 기준이 생겼다. “예쁜 이미지”를 만드는 데서 끝나지 않고, 본문 메시지를 잘 압축했는지, 이전 장의 톤과 이어지는지, 이미지 안의 한글이 맞는지, 잘림이나 워터마크가 없는지 확인해야 했다.
...
반복 업무가 Skill로 바뀌는 핵심 순간은 성공했을 때보다 실패했을 때 더 자주 온다. 예를 들어 요약이 매번 길어지거나, 조사 결과가 링크 목록으로 끝나거나, WikiDocs 발행 뒤 공개 페이지 확인이 빠진다면 그건 단순 실수가 아니라 절차가 부족하다는 신호다.

이때 바로 memory에 긴 설명을 넣거나 새 자동화를 만들면 안 된다. 먼저 실패를 한 문장으로 이름 붙이고, 원인을 절차/입력/검증/저장 위치 중 어디에 둘지 나눈다. 그런 다음 다음 실행에서 확인할 기준을 Skill 후보로 올린다.

```text
현재 결과를 고친다
→ 실패를 한 문장으로 정의한다
→ 원인을 절차 / 입력 / 검증 / 저장 위치로 나눈다
→ Skill 또는 체크리스트에 반영한다
→ 다음 실행에서 같은 실패가 줄었는지 확인한다
```
