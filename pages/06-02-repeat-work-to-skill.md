...
WikiDocs 발행은 처음엔 단순해 보였다. 글을 고치고, 원격 저장소에 반영하면 끝나는 일처럼 보였다. 하지만 실제 운영에서는 TOC 링크, H1 금지, 이미지 경로, 중점 문자, raw `.md` body 링크, WikiDocs 동기화 확인까지 함께 봐야 했다.

여기에 SEO/GEO 기준이 더해졌다. 검색자가 `에르메스 에이전트란 무엇인가`로 들어왔을 때 첫 문단에서 답을 얻는지, `Hermes Agent`와 `에르메스 에이전트`가 자연스럽게 같이 쓰이는지, 본문 안의 개념 링크가 독자 이동을 돕는지 확인해야 했다. 이 기준은 [GitHub/WikiDocs 발행 케이스](https://wikidocs.net/345994)와 이어진다.
...
Skill은 오래 살아남는 절차여야 한다. 바뀌는 값은 source of truth에서 조회하고, Skill에는 조회하는 순서와 판단 기준만 남긴다.
...
### 작업 로그를 Skill에 넣어도 되나요?

넣지 않는 편이 좋다. 작업 로그는 session_search, GitHub log, Obsidian, shared-memory 같은 곳에서 회수하고, Skill에는 반복 가능한 절차만 남긴다.

## 다음 글

다음은 [역할형 에이전트별 Skill은 어떻게 나눌까](https://wikidocs.net/346237)에서 같은 Skill을 모든 에이전트가 공유해도 되는지, 하비/방울이/뽀동이/하망이처럼 역할별로 나눠야 하는지 살펴본다.
