...
에르메스 에이전트(Hermes Agent)를 처음 검색하는 독자는 보통 “이게 챗봇인가, 자동화 도구인가, AI 에이전트 프레임워크인가”를 먼저 알고 싶어 한다. 1장의 답은 분명하다. Hermes Agent는 좋은 답변을 한 번 받는 도구가 아니라, AI 개인비서와 역할형 에이전트로 실제 업무 흐름을 굴리기 위한 운영 구조다.
...
1장은 그 검색 초입 질문을 받는 관문이다. 챗봇과 [AI 개인비서](https://wikidocs.net/345923)가 어떻게 다른지, 혼자 일해도 왜 [AI 팀 구조](https://wikidocs.net/345924)가 필요한지, 하비/방울이/뽀동이 같은 운영 이름을 어떻게 읽으면 되는지 먼저 정리한다. 뒤에서 나오는 MCP, cron, skill, memory, WikiDocs 같은 기능은 [에르메스 에이전트(Hermes Agent)의 핵심 개념](https://wikidocs.net/346055)과 [운영 시스템 관점](https://wikidocs.net/345890) 위에서 이해하면 훨씬 덜 복잡하다.
...
| 01-5 | 에르메스 에이전트(Hermes Agent)는 왜 운영 시스템인가 | 대화, 실행, 기록, 재사용이 이어지는 구조 |
