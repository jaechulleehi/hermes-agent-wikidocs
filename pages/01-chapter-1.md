...
AI를 업무에 붙이다 보면 처음에는 “좋은 답변을 받는 법”에 집중하게 된다. 하지만 반복 업무가 늘어나면 질문 하나를 잘 던지는 것만으로는 부족해진다. 조사해야 할 것이 있고, 정리해야 할 문서가 있고, 실행해야 할 작업이 있고, 나중에 다시 꺼내볼 기록도 필요하다.

그래서 이 책은 AI를 챗봇 하나로 보지 않고, **AI 개인비서와 역할형 에이전트가 함께 움직이는 업무 흐름**으로 본다. 사용자는 모든 작업을 세세히 지시하고 싶지 않다. 하나의 입구에 요청하면, 필요한 역할이 나뉘고, 결과가 다시 읽기 좋은 형태로 돌아오기를 원한다.

1장은 그 관점 전환을 다룬다. 챗봇과 AI 개인비서가 어떻게 다른지, 혼자 일해도 왜 AI 팀 구조가 필요한지, 하비/방울이/뽀동이 같은 운영 이름을 어떻게 읽으면 되는지 먼저 정리한다. 뒤에서 나오는 MCP, cron, skill, memory, WikiDocs 같은 기능은 이 기본 구조 위에서 이해하면 훨씬 덜 복잡하다.
...
| 01-4 | Hermes Agent 주요 개념과 기능은 어떻게 연결될까 | 주요 기능을 하나의 운영 흐름으로 보는 지도 |
