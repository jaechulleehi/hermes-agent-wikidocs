...
그래서 기억 시스템은 저장소 크기보다 역할 분리가 먼저다. 항상 필요한 짧은 기준은 memory에 두고, 팀이 함께 봐야 하는 작업 원본은 shared-memory에 둔다. 반복 실행 절차는 skills로 만들고, 누적 지식과 운영 기록은 Obsidian 같은 외부 지식 베이스에 남긴다. 과거 대화는 session_search로 회상하고, 넓은 문서 검색이 필요할 때는 OpenViking 같은 RAG 인덱스를 붙인다.

예를 들어 한 팀이 개인 비서형 에이전트, 조사형 에이전트, 정리형 에이전트를 함께 운영한다고 하자. 내부에서는 하비, 방울이, 뽀동이 같은 이름을 붙일 수 있다. 다만 독자가 먼저 이해해야 할 것은 이름보다 역할이다. 누가 새 정보를 받아 분류하고, 누가 원자료를 조사하고, 누가 읽기 쉬운 글로 바꾸며, 어떤 지식이 장기 기억으로 승격되는지를 나누는 것이 기억 관리 아키텍처의 핵심이다.
...
| Obsidian | 사람이 읽고 에이전트가 다시 찾는 지식 본체 | 프로젝트 지식, 개념, 결정 기록 | 정제되지 않은 원문 더미 |
| session_search | 과거 대화 회상 | 이전 논의, 지나간 작업 맥락 | 장기 기준의 대체물 |
| OpenViking/RAG | 검색과 회수 인덱스 | Obsidian 기반 검색 대상 | 검토되지 않은 원문 더미 |

## Obsidian은 함께 관리하는 지식 본체, OpenViking은 검색 인덱스다

Obsidian은 사람만 보는 메모장도 아니고, 에이전트만 쓰는 데이터 저장소도 아니다. 사람이 읽고 고치며, Hermes Agent가 다시 회수할 수 있게 함께 관리하는 공동 지식 본체다. 프로젝트의 원칙, 회의에서 나온 결정, 콘텐츠 자산, 사용자 피드백, 운영 기록은 여기에 남긴다.
...
| Storage History | 무엇을 어디에 왜 저장했는지 기록 |
| Slack Reporting | 운영 상태와 다음 행동을 짧게 보고 |

## 저장 이력이 필요한 이유

지식 시스템은 시간이 지나면 “왜 이게 여기 있지?”라는 질문이 생긴다. 저장 이력은 저장 자체보다 저장한 이유를 남긴다.

한 줄짜리 기록만 있어도 나중에 다음을 추적할 수 있다.
...
## 외부에 설명할 때는 이름보다 역할을 먼저 보여준다

내부 운영에서는 에이전트마다 이름을 붙이는 것이 좋다. 이름이 있으면 사람이 부르기 쉽고, 역할도 기억하기 쉽다. 다만 책이나 팀 문서로 설명할 때는 이름을 바로 앞세우기보다 역할을 먼저 풀어주는 편이 읽기 쉽다.
...
좋은 구조는 에이전트가 모든 것을 혼자 기억하게 만드는 것이 아니라, 사람과 에이전트가 함께 관리한 지식을 필요한 순간에 꺼낼 수 있게 만든다. 그래서 기억 관리 아키텍처의 핵심은 저장량이 아니라 분류, 정제, 승격, 검증, 재사용의 운영 흐름이다.
