...
## 프로젝트: 크론 조사에서 WikiDocs 발행까지

이 프로젝트에서는 매일 또는 매주 반복되는 조사 주제를 cron으로 시작하고, 사람이 판단할 수 있는 요약으로 받은 뒤, 가치 있는 신호만 WikiDocs 원고로 전환한다.
...
| 준비 항목 | 설명 |
|---|---|
| 검색 도구 | 웹 검색 MCP, 브라우저 검색, RSS, 공식 docs 확인 경로 중 하나가 필요하다 |
...
- 본문 내부 링크가 WikiDocs 공개 URL로 연결되는가
- 중점 문자가 남아 있지 않은가
- 보호해야 할 정보가 드러나지 않는가
- 첫 문단이 검색자 질문에 답하는가
```
...
### 어떤 주제가 WikiDocs 후보인가요?

반복 가능한 운영 기준이 생겼거나, 독자가 따라 할 수 있는 설정/체크리스트/실패 패턴이 있는 주제다. 단순 뉴스 요약은 WikiDocs보다 Slack 브리핑이나 블로그 후보에 가깝다.
