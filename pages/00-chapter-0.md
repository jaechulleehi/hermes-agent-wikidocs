...
이 책은 공식 문서를 대체하지 않는다. 설치 명령, CLI 옵션, 설정값처럼 바뀔 수 있는 내용은 공식 GitHub와 문서를 기준으로 확인한다. 이 책은 그 기능을 실제 업무에 붙일 때의 순서와 판단 기준을 다룬다. 기억할 정보는 memory/shared-memory에 남기고, 반복 절차는 Skill로 만들고, 과거 작업은 session search로 찾고, 정기 작업은 cron으로 시작하고, 외부 업무 시스템은 MCP/gateway로 연결한다. 이 흐름을 잡아야 이후 장의 AI 개인비서, 역할형 에이전트, 기억 시스템, Skill 운영이 하나의 구조로 이어진다.
