...
| 순서 | 페이지 | 먼저 답하는 질문 |
|---|---|---|
| 00-1 | 에르메스 에이전트(Hermes Agent)란 무엇인가 | Hermes Agent가 챗봇/IDE copilot과 무엇이 다른가 |
| 00-2 | 공식 GitHub/docs는 어디서 볼까 | 공식 source of truth와 이 책의 역할은 어떻게 다른가 |
| 00-3 | 설치와 세팅은 어떻게 시작할까 | 설치 명령보다 먼저/나중에 무엇을 검증해야 하는가 |
| 00-4 | CLI 첫 대화는 어떻게 시작할까 | 설치 후 첫 대화, 세션, slash command를 어떻게 확인하는가 |
| 00-5 | provider/model/config는 어떻게 확인할까 | 모델 라우팅과 설정 파일은 어디서 흔들리는가 |
| 00-6 | OpenClaw와 무엇이 다를까 | 기존 환경에서 Hermes로 넘어올 때 무엇을 버리고 가져올까 |
| 00-7 | Claude Code/Codex와 어떻게 다를까 | 코딩 에이전트와 운영형 개인비서는 어떻게 다르게 쓸까 |
| 00-8 | Docker/Gateway는 언제 필요할까 | 로컬 실습과 상시 운영의 경계는 어디인가 |
| 00-9 | 어떤 채널에서 쓸 수 있을까 | CLI/Slack/Telegram/Discord/Webhook 중 어디서 부를까 |
...
먼저 [에르메스 에이전트(Hermes Agent)란 무엇인지](https://wikidocs.net/346055) 확인한다. 그다음 공식 GitHub/docs, 설치, CLI 첫 대화, provider/model/config 순서로 보면 0장이 하나의 체크리스트처럼 작동한다.
