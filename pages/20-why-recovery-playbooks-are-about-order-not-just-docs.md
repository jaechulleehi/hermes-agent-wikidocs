...
![복구 플레이북에서 순서를 먼저 정하는 흐름](../assets/how-image-agent-creates-wikidocs-visuals/ch08-2-recovery-playbook-order-codex.png)
...
예를 들어 [gateway](https://wikidocs.net/345906)가 응답하지 않을 때 바로 재설치부터 하면 process 문제인지, 환경 변수 문제인지, profile 문제인지 구분하기 어렵다. WikiDocs가 갱신되지 않았을 때도 바로 본문을 다시 쓰면 GitHub push 문제인지, WikiDocs sync 문제인지, TOC 문제인지 흐려진다.
...
| 2 | 최근 변경 | 마지막으로 바뀐 commit, config, cron, skill을 본다 |
| 3 | process/runtime | 실제 실행 중인지와 어느 profile에서 도는지 본다 |
| 4 | 권한/경로 | 파일, 토큰, delivery target, 작업 디렉터리를 확인한다 |
...
- 공개 문서에는 내부 경로나 비밀값이 아니라 복구 판단 순서만 남긴다.
...
복구 순서를 운영 체크리스트로 고정하는 방법은 [Hermes Agent 운영 체크리스트는 어떻게 써야 할까](https://wikidocs.net/345919)에서 이어서 본다.
