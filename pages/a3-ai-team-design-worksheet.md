...
처음부터 여러 에이전트를 만들지 말고 한 주에 하나씩 운영 층을 추가한다.

| 주차 | 목표 | 채워 넣을 것 |
|---|---|---|
| 1주차 | AI 개인비서 한 명으로 반복 업무 하나를 처리한다 | 메인 창구, 호출 채널, 첫 완료 기준 |
| 2주차 | 피드백을 기억과 절차로 나눈다 | memory/user profile, Skill 후보, 수정 기준 |
| 3주차 | 두 번째 역할형 에이전트를 붙인다 | 조사형→정리형 또는 정리형→실행형 handoff, source of truth |
| 4주차 | 자동화와 안전 점검을 붙인다 | cron, delivery, gateway/log 점검, 권한/보안 체크리스트 |

각 주차가 끝날 때 “에이전트가 하나 늘었는가”보다 “반복 업무가 더 안정적으로 끝났는가”를 확인한다.
