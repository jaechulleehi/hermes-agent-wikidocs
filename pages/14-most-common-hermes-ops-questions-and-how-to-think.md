...
![Hermes Agent 운영 질문을 레이어별로 먼저 분류하는 흐름](../assets/images/body-figures/ch08-1-ops-question-layer-router-codex-only.webp)
...
WikiDocs 본문 링크 문제는 처음에는 “링크가 이상하다”로 보였다. 하지만 실제 원인은 글쓰기 문제가 아니라 publishing/원본 기준 레이어였다. GitHub 본문에는 `.md` 링크가 자연스럽지만, GitHub 연동 WikiDocs 화면에서는 공개 페이지 ID 링크가 필요했다. 그래서 해결책도 단순 문장 수정이 아니라 WikiDocs TOC에서 page ID를 확인하고 본문 링크를 `https://wikidocs.net/{page_id}`로 바꾸는 것이었다.
...
5. 책이나 팀 문서로 정리할 때는 읽는 사람이 몰라도 되는 불필요한 세부값을 빼고 판단 기준만 남긴다.
