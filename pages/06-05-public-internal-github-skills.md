## 공개 GitHub Skill과 내부 Skill은 어떻게 분리할까

에르메스 에이전트(Hermes Agent)의 Skill은 공개해서 다른 사람이 재사용할 수도 있고, 개인이나 팀 내부에만 둘 수도 있다. 둘을 나누는 기준은 “멋있어 보이는가”가 아니라 보호해야 할 정보, 조직 맥락, 재사용 가능성, 검증 가능성이다.

![공개 GitHub Skill과 내부 Skill 분리](../assets/images/chapter-heroes/ch6-5-public-internal-github-skills-codex.webp)

공개 GitHub Skill은 일반화된 절차여야 한다. 내부 Skill은 특정 사용자, 팀, 프로젝트, 채널, 계정, 원본 기준에 맞춘 판단 기준을 담을 수 있다. 이 구분은 [memory/profile/session 경계](https://wikidocs.net/346126)와도 이어진다. 두 경계를 섞으면 밖에 나가면 안 되는 정보가 섞이거나, 반대로 내부 운영에 필요한 기준이 너무 추상화되어 쓸모가 약해진다.

[TOC]

## 공개 Skill과 내부 Skill의 차이

| 구분 | 공개 GitHub Skill | 내부 Skill |
|---|---|---|
| 목적 | 다른 사용자도 재사용할 수 있는 일반 절차 | 특정 팀/사용자/프로젝트의 판단 기준 |
| 넣을 수 있는 것 | 도구 사용 순서, 검증 기준, 흔한 실패 | 팀 경로, 보고 스타일, 원본 기준 우선순위 |
| 넣으면 안 되는 것 | 계정, 토큰, 내부 경로, 실명, 비공개 정책 | 토큰/비밀번호 같은 비밀값은 여기도 금지 |
| 검증 | 누구나 따라 할 수 있는 기준 | 팀 원본 기준과 실제 사용 기준 |
| 예시 | GitHub PR 리뷰 절차, YouTube transcript 정리 | Hermes WikiDocs 발행 기준, 사용자별 보고 스타일 |

내부 Skill이라고 해서 보호해야 할 정보를 넣어도 되는 것은 아니다. token, password, webhook, private key는 Skill이 아니라 안전한 credential store나 환경 설정에서 관리해야 한다.

## 사례: WikiDocs Skill은 어디까지 공유할 수 있나

뽀동이의 WikiDocs 발행 Skill에는 함께 공유할 수 있는 부분과 내부에 남겨야 할 부분이 함께 있다. H1 금지, 이미지 경로 확인, TOC 링크 검증, SEO/GEO 첫 문단 점검은 재사용 가능한 일반 기준이다. 하지만 특정 repo 경로, 개인 채널, 사용자별 문체 선호, GitHub 인증 우회 방법 같은 운영 세부값은 그대로 드러내면 안 된다.

그래서 책에서는 “GitHub가 원본 기준이고 WikiDocs는 배포 채널이다” 같은 구조를 설명하고, 내부 Skill에는 실제 repo 위치, 반영 확인 방식, 보고 형식처럼 작업자가 실수하지 않기 위해 필요한 구체 기준을 둔다. 책에 넣을 내용은 재사용 가능한 원칙으로 바꾸고, 내부 Skill은 실제 작업자가 따라 할 수 있게 구체화한다.

## 사례: 하망이 이미지 Skill은 무엇을 공유할 수 있나

하망이 이미지 제작 흐름도 마찬가지다. 공개 Skill에는 “[본문 메시지를 먼저 읽고 시각 콘셉트를 잡는 흐름](https://wikidocs.net/345991)”과 “이미지 안의 한글과 잘림을 vision으로 검수한다”는 절차를 넣을 수 있다. 하지만 특정 계정, 내부 출력 경로, 팀 전용 캐릭터 자산, 배포 채널 세부값은 내부 기준으로 남긴다.

이 구분이 있어야 다른 사람은 이미지형 에이전트 운영법을 배울 수 있고, 팀 내부 작업자는 실제 배포 실수를 줄일 수 있다.

![공개 Skill과 내부 Skill의 분리 기준](../assets/images/body-figures/ch06-05-public-internal-skill-boundary-codex-only.webp)

## 외부 Skill을 내부 운영에 맞게 바꾸는 순서

공개 Skill은 출발점이지 완성본이 아니다. 특히 다른 에이전트 환경에서 온 Skill은 Hermes Agent의 toolset, profile, memory, approval 규칙에 맞게 번역해야 한다.

1. 원본 Skill의 목적을 한 문장으로 줄인다.
2. Hermes Agent에서 실제로 쓸 tool 이름으로 바꾼다.
3. 로컬 경로, 계정명, token 예시를 제거한다.
4. 위험 명령은 approval 기준과 sandbox 기준을 붙인다.
5. 마지막에 검증 명령과 실패 시 되돌리는 방법을 넣는다.

이 과정을 거치면 공개 Skill은 내부 운영 절차가 되고, 내부 절차는 나중에 다시 공유할 수 있는 형태로 다듬을 수 있다.

## 공유하기 전 점검 기준

1. token, password, webhook, private key가 없는가?
2. 개인 경로, 채널 ID, 실명, 계정명이 남아 있지 않은가?
3. 특정 팀만 아는 약어를 일반 독자도 이해할 수 있게 바꿨는가?
4. 실패 사례를 개인 탓이 아니라 판단 기준으로 바꿨는가?
5. 검증 방법이 외부 사용자도 따라 할 수 있는가?
6. 책 본문과 내부 Skill의 원본 기준이 분리되어 있는가?

## GitHub 공개 Skill을 보강하는 흐름

공개 Skill은 일반 독자에게 도움이 되는 기준만 남긴다. 내부 시행착오를 그대로 복사하지 말고, 문제/착각/판단 기준/검증 방법으로 다시 쓴다. 예를 들어 “특정 사용자가 이렇게 선호한다”는 문장은 공개 Skill에 바로 넣지 않는다. 대신 “[사용자별 보고 형식](https://wikidocs.net/346126)은 memory에 두고, Skill에는 보고 항목의 구조를 둔다”처럼 기준으로 바꾼다.

공개 Skill을 보강한 뒤에는 README, 예시, 검증 방법이 함께 맞는지 확인한다. Skill은 도구 설명서가 아니라 실제 반복 작업을 안정화하는 운영 단위이기 때문이다.

## FAQ

### 내부 Skill을 나중에 공개 Skill로 바꿀 수 있나요?

가능하다. 다만 그대로 옮기지 말고 보호해야 할 정보를 제거한 뒤, 특정 팀의 맥락을 일반적인 판단 기준으로 다시 써야 한다.

### 공개 Skill에는 실제 사례를 넣으면 안 되나요?

넣어도 된다. 다만 개인, 고객, 계정, 경로, 비밀값을 제거하고 재사용 가능한 문제와 기준만 남겨야 한다.
