...
3장은 일을 **[조사형/정리형/실행형](https://wikidocs.net/345925)**으로 나누는 기준을 다룬다. 역할을 나누는 목적은 에이전트 수를 늘리는 것이 아니라 실패 지점을 잘 보이게 만드는 것이다. 근거가 부족한 문제인지, 구조가 약한 문제인지, 실행과 검증이 빠진 문제인지 구분할 수 있어야 한다.
...
역할형 에이전트를 나눈다고 해서 모든 문제가 자동으로 해결되지는 않는다. 흔한 착각은 세 가지다.
