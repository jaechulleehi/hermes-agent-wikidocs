...
이 페이지는 책 전체에서 반복해서 나오는 주요 개념과 기능을 한 번에 연결하는 안내판이다. 독자는 여기서 큰 지도를 보고, 뒤의 장에서는 각각의 개념이 실제 운영에서 어떻게 문제를 만들고 해결되는지 따라가면 된다.
...
이번 WikiDocs 책 보강 작업을 예로 들면 기능들이 이런 순서로 묶였다.

```text
Slack 요청
→ 세션 기록 검색
→ shared-memory와 source map 확인
→ WikiDocs 페이지 수정
→ 내부 링크를 WikiDocs page URL로 변환
→ 검증 스크립트 실행
→ GitHub commit/push
→ 공개 페이지 확인
```

여기에는 여러 기능이 섞여 있다. Slack은 요청이 들어오는 창구이고, session search는 과거 맥락을 회수하는 도구이며, shared-memory는 팀 공통 기준을 확인하는 장소다. GitHub는 원본 이력을 남기는 source of truth이고, WikiDocs는 독자가 읽는 공개 채널이다.

이 흐름에서 민감정보는 본문으로 옮기지 않는다. 공개 문서에는 “왜 raw Markdown 링크가 WikiDocs 화면에서 안 먹었는지”, “왜 page ID 기반 링크로 바꿨는지”, “왜 GitHub를 원본으로 두는지” 같은 운영 판단만 남긴다. 토큰, 계정, 개인 경로, 채널 ID 같은 값은 독자에게 필요하지 않다.
...
공개 사례를 써야 한다 → 이야기/판단 기준만 남기고 민감정보는 제거했는가?
```
