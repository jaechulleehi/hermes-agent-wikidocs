...
에르메스 에이전트(Hermes Agent)는 단순히 질문에 답하는 AI 챗봇이 아니라, 메모리, 프로필, 스킬, cron, MCP, gateway 같은 기능을 묶어 실제 업무 흐름을 운영하게 돕는 오픈소스 AI 에이전트이자 AI 에이전트 프레임워크다. 이 책에서는 에르메스 에이전트(Hermes Agent)를 설치 기능보다 **AI 개인비서와 역할형 에이전트로 업무 자동화를 굴리는 운영 시스템**으로 설명한다.
...
다음에는 [Hermes Agent 공식 GitHub와 공식 문서](https://wikidocs.net/346136)를 어디서 확인해야 하는지 본다. 공식 기준을 먼저 잡아야 이 책의 운영 해석도 흔들리지 않는다.
