...
처음 읽는 독자라면 먼저 이것만 잡으면 된다. Hermes Agent의 핵심은 “더 긴 답변을 받는 법”이 아니라, 요청을 받고, 역할을 나누고, 도구를 연결하고, 결과를 기록으로 남기는 흐름을 만드는 데 있다.

![Hermes Agent 핵심 운영 루프](../assets/images/body-figures/ch00-core-concepts-operating-loop-codex-only.webp)

[TOC]

## 한 문장으로 정의하면

에르메스 에이전트(Hermes Agent)는 AI 개인비서가 메인 창구가 되고, 조사형/정리형/실행형 에이전트와 외부 도구가 함께 움직이도록 만드는 업무 자동화 운영 구조다. 처음에는 Slack, Telegram, CLI 같은 대화창으로 보이지만, 그 대화창은 입구일 뿐이다. 실제 핵심은 요청을 해석하고, 필요한 도구를 쓰고, 오래 남길 기준을 기억하고, 반복 절차를 Skill로 개선하는 운영 흐름에 있다.
...
Hermes Agent를 처음 배울 때 모든 기능 이름을 한 번에 외우려고 하면 어렵다. 먼저 아래 다섯 층만 잡으면 된다.

| 층 | 독자가 이해할 말 | 대표 기능 |
|---|---|---|
| 메시지/프로필 | 어디에서 누구에게 요청하는가 | CLI, Slack, Telegram, profile |
| 기억/회상 | 오래 남길 기준과 다시 찾아올 기록을 나눈다 | memory, user profile, session search |
| 도구/확장 | 말이 아니라 실제 확인과 실행을 한다 | file, terminal, browser, MCP, plugin |
| 파일/작업 대상 | 무엇을 읽고 어디에 결과를 남길지 정한다 | Obsidian, GitHub, WikiDocs, 로컬 파일 |
| 검증/개선 | 결과를 확인하고 반복 절차로 남긴다 | diff, test, log, Skill, cron |

이 다섯 층을 잡으면 Hermes는 특정 메신저 봇이 아니라 `대화 / 기억 / 도구 / 파일 / 검증`을 연결하는 AI 작업 환경으로 보인다.
...
## 좋은 요청은 프롬프트가 아니라 작업 명세다

Hermes에게 일을 맡길 때는 멋진 문장보다 작업 범위가 중요하다. 입문자는 아래 여섯 가지만 적어도 결과 품질이 크게 안정된다.
...
금지 조건: 삭제, 외부 전송, 결제, 대량 수정처럼 하지 말아야 할 일은 무엇인가
```

이 구조는 단순한 프롬프트 템플릿이 아니다. AI 개인비서에게 작은 업무 티켓을 주는 방식이다. 목표와 입력, 산출물, 검증 기준이 분명할수록 Hermes는 답변 생성기가 아니라 작업 파트너에 가까워진다.
