...
## Hermes의 강점은 무오류가 아니라 검증 루프다

Hermes가 도구를 쓴다고 해서 모든 결과가 자동으로 맞아지는 것은 아니다. 웹 검색 결과가 오래됐을 수 있고, 파일 수정이 의도한 범위를 벗어날 수 있으며, API 응답은 성공처럼 보여도 실제 공개 화면은 아직 바뀌지 않았을 수 있다.

Hermes의 장점은 틀리지 않는다는 데 있지 않다. 틀릴 가능성을 줄이기 위해 `읽기 / 실행 / 재확인 / 기록`을 같은 작업 흐름 안에 넣을 수 있다는 데 있다. 그래서 도구 실행 뒤에는 항상 사용자가 실제로 필요로 한 결과가 나왔는지 확인해야 한다.
