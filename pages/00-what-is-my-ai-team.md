...
나만의 AI 팀은 여러 에이전트를 많이 만드는 구조가 아니다. 핵심은 업무를 **조사/정리/실행/기록** 같은 역할로 나누고, 하나의 AI 개인비서가 그 흐름을 묶게 하는 것이다.

혼자 일하는 사람에게도 역할 분리는 필요하다. 실제 업무는 한 종류의 작업으로 끝나지 않는다. 자료를 찾고, 판단하고, 문서로 만들고, 도구를 실행하고, 결과를 다시 기록으로 남겨야 한다. 이 모든 일을 한 대화 안에서 즉흥적으로 처리하면 빠를 수는 있지만 반복할수록 품질이 흔들린다.
...
| 공개 개념 | 운영 예시 이름 | 주 역할 | 흔한 실패 | 확인 기준 |
|---|---|---|---|---|
...
예를 들어 “새로운 주제로 공개 글을 만들고 싶다”는 요청이 있다고 하자. 이 요청은 단순 글쓰기 요청처럼 보이지만 실제로는 여러 단계로 나뉜다.
...
실행형은 파일 수정, 명령 실행, 배포처럼 상태를 바꾸는 일을 맡기 때문이다. 작업 범위와 검증 기준이 없으면 빠르게 실행하는 만큼 위험도 커진다.

### 역할 분리만 하면 자동화가 완성되나요?

아니다. 역할 분리는 시작점이다. 반복되는 절차는 skill이나 체크리스트로 남기고, 정기 작업은 cron으로 돌리고, 결과는 위키나 문서로 남겨야 운영이 된다.
