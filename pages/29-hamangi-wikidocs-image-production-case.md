...
![하망이 WikiDocs 이미지 제작 운영 흐름](../assets/images/body-figures/ch11-01-hamangi-wikidocs-image-production-codex-only.webp)
...
하망이 사례의 핵심 규칙은 하나다. 이미지는 예쁜 결과물을 고르는 작업이 아니라, 본문 메시지를 압축하고 오해를 줄이는 QA 작업이다. 처음에는 “그럴듯한 이미지가 있으면 장이 살아난다”고 생각하기 쉽지만, 실제 운영에서는 이미지가 본문과 따로 놀거나, 작은 한국어 텍스트가 깨지거나, asset 경로가 맞지 않아 공개 화면에서 보이지 않는 문제가 더 자주 생긴다.

그래서 이 케이스는 이미지 제작을 `메시지 압축 → 제작 방식 선택 → 생성/렌더링 → vision QA → assets 연결 → WikiDocs 공개 확인`으로 나눈다. 독자가 따라 할 때도 이미지 프롬프트부터 쓰지 말고, 먼저 페이지가 말하려는 한 문장을 정해야 한다.

| 처음에 흔히 막히는 지점 | 운영 규칙 | 따라 하는 방법 |
|---|---|---|
| 분위기는 좋은데 본문과 연결되지 않는다 | 이미지보다 메시지를 먼저 정한다 | 첫 문단을 한 문장으로 줄인 뒤 프롬프트에 넣는다 |
| 한글 텍스트가 깨지거나 작다 | 이미지 안의 긴 문장을 줄인다 | 텍스트는 본문이 맡고 이미지는 구조와 관계를 보여준다 |
| 생성한 파일이 공개 화면에서 안 보인다 | 제작과 발행을 한 작업으로 묶는다 | `assets/` 경로, Markdown 상대 경로, WikiDocs 화면을 함께 확인한다 |
...
```text
이미지형 에이전트는 Hermes Agent에서 장식 담당이 아니라
본문 메시지를 시각적으로 압축하고 검증까지 맡는 역할이다.
```
