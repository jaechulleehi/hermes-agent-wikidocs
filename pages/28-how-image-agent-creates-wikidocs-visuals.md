...
WikiDocs 본문 이미지는 장식이 아니다. 독자가 긴 설명을 읽기 전에 핵심 구조를 한 번에 잡게 만드는 안내판이다. 그래서 이미지형 에이전트인 하망이는 예쁜 그림을 만드는 역할이 아니라, 글의 메시지를 시각 구조로 바꾸는 역할을 맡는다.

우리 운영에서 하망이는 썸네일, 설명 이미지, HTML-to-PNG, Codex imagegen, PIL 후처리 같은 제작 흐름을 담당한다. 이 역할은 [조사형/정리형/실행형 에이전트](https://wikidocs.net/345898) 흐름에서 이미지형 에이전트로 확장된 사례다. 하지만 최종 기준은 항상 메시지다. 디자인이 좋아도 본문 구조와 맞지 않으면 다시 만든다.
...
4장 이미지는 memory/session/profile/source of truth 경계를 보여줘야 했고, 6장 이미지는 WikiDocs/블로그/강의 콘텐츠 시스템의 흐름을 보여줘야 한다. 즉 이미지는 페이지의 핵심 판단 기준을 압축해야 한다.
...
아니다. 오해 요소, 텍스트 잘림, 정보 과밀, 본문과의 연결을 검수해야 한다.

## 다음 글

다음은 [운영 FAQ/실패 패턴/멀티봇 규칙](https://wikidocs.net/345916)에서 이런 운영 흐름이 여러 봇과 스레드에서 어떻게 흔들리는지 본다.
