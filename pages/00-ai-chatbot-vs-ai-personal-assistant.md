...
Hermes Agent를 업무에 붙일 때 중요한 차이는 여기에 있다. 좋은 답변을 받는 것에서 끝낼지, 아니면 조사/정리/실행/기록이 이어지는 반복 가능한 업무 흐름을 만들지 먼저 정해야 한다.
...
| 이전 작업 이어서 해줘 | 현재 대화에서만 찾는다 | 세션 기록, 문서, GitHub, 위키 등 source of truth를 확인한다 |
| 공개 문서로 정리해줘 | 내용을 보기 좋게 요약한다 | 독자에게 필요한 이야기와 빼야 할 내부 정보를 구분한다 |

AI 개인비서는 모든 일을 직접 하는 존재가 아니다. 오히려 “이 일은 조사형에게, 이 일은 정리형에게, 이 일은 실행형에게”처럼 역할을 나누고 최종 결과를 묶는 입구에 가깝다.
...
- 여러 도구나 문서를 확인해야 하는가?
- 공개 문서로 바꿀 때 독자에게 필요한 정보와 불필요한 정보를 나눠야 하는가?
...
### 역할형 에이전트는 꼭 여러 개로 나눠야 하나요?

처음부터 많이 나눌 필요는 없다. 다만 조사, 정리, 실행이 자주 섞인다면 최소한 역할 기준은 나눠두는 편이 좋다.

### Hermes Agent의 공식 기능은 이 책에서 어떻게 다루나요?

기능 목록으로 나열하지 않는다. 실제 업무 흐름 안에서 memory, skill, cron, MCP 같은 기능이 왜 필요한지 설명한다.
