...
이 구조를 쓰는 이유는 단순하다. 공개 글은 한 번 쓰고 끝나지 않는다. 제목, TOC, 이미지, SEO/GEO, 전자책 규칙, 내부 링크가 계속 바뀐다. 이력과 기준이 남아야 안전하게 고칠 수 있다. 그래서 GitHub/WikiDocs 발행은 7장의 [WikiDocs를 먼저 쓰는 콘텐츠 시스템](https://wikidocs.net/345908)을 운영으로 고정하는 단계다.
...
| 안전 | 비공개 내부값과 독자가 볼 필요 없는 운영 흔적이 제거되었는가 |
...
항상은 아니다. 하지만 새 운영 케이스가 추가되거나 공식 docs 매핑이 바뀌거나 장의 역할이 달라지면 원본 자료 목록과 책 구조 기준도 함께 봐야 한다.
