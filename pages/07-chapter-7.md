...
[Hermes Agent](https://wikidocs.net/346055)를 AI 개인비서와 역할형 에이전트 팀으로 운영하면 운영 FAQ는 기능별 질문이 아니라 실패 패턴을 가르는 기준이 된다. Slack에서는 “이거 누가 해야 해?”, “왜 같은 하비인데 다르게 답하지?”, “방울이와 뽀동이를 같이 불러도 되나?”처럼 섞인 질문으로 들어온다. 그래서 Hermes Agent 운영 FAQ는 답변집이 아니라 문제를 gateway, memory, profile, Skill, 멀티봇 호출 규칙 중 어느 레이어에서 볼지 정하는 라우터여야 한다.
...
| 산출물 | 확인 질문 |
|---|---|
| 운영 질문 분류표 | 질문을 identity/profile/role/tool/runtime/source of truth/recovery 중 어디에 둘지 보이는가 |
| 멀티봇 호출 규칙 | 자동 응답할 상황과 침묵할 상황이 구분되어 있는가 |
| FAQ→체크리스트 전환 기준 | 반복 질문이 단순 답변이 아니라 실행 순서로 내려가는가 |
| 스레드 정리 기준 | 하비가 창구를 잡고 보조 에이전트가 필요한 때만 호출되는가 |

## 실제 운영에서 드러난 장면

WikiDocs 작업 스레드는 좋은 예다. 사용자는 한 스레드에서 “전체 읽는 흐름을 봐줘”, “본문 링크가 없다”, “블로그 원문과 공식 docs가 잘 옮겨졌는지 봐줘”, “내용이 축약되어 보인다”를 한꺼번에 말했다. 이 요청은 단순 작성 요청이 아니었다. [AI 개인비서 메인 창구](https://wikidocs.net/345892)가 먼저 범위를 잡고, 본문 링크 기준을 다시 해석하고, 세션 기록과 운영 기록을 찾고, 뽀동이가 책형 구조로 리라이트해야 하는 복합 운영 요청이었다.
...
이 기준은 읽는 사람에게도 그대로 적용된다. AI 팀을 만들 때 “봇을 많이 붙이면 좋다”가 아니라 “봇이 언제 침묵해야 하는지 정해야 한다”가 먼저다. 침묵 규칙이 없으면 메인 창구가 사라지고, 대화는 업무 공간이 아니라 알림 소음이 된다.
