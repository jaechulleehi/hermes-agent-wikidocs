...
![Slack 스레드에서 하비가 일을 분배하는 흐름](../assets/images/body-figures/ch11-05-slack-thread-harvey-delegation-codex-only.webp)
...
Slack 스레드 사례의 핵심 규칙은 “빠른 대화일수록 창구, 역할, 저장 위치, 완료 기준이 필요하다”는 것이다. 처음에는 사용자가 떠올린 내용을 바로 말하고, 여러 에이전트가 각자 도와주면 빨라질 것처럼 보인다. 하지만 실제 운영에서는 사람 대화와 봇 응답이 섞이고, 누가 최종 판단자인지 흐려지고, 결과가 Slack 안에만 남아 다음 작업에서 다시 설명해야 하는 일이 생긴다.

그래서 하비를 메인 창구로 두고, 보조 에이전트는 필요한 때만 불러야 한다. 하비는 요청을 `목적/역할/결과물/저장 위치/완료 기준`으로 나누고, 마지막에는 완료/남음/다음을 짧게 묶어야 한다. 이 규칙이 있어야 Slack 스레드가 잡담이 아니라 업무 라우터가 된다.

| 처음에 흔히 막히는 지점 | 운영 규칙 | 따라 하는 방법 |
|---|---|---|
| 여러 봇이 동시에 답해 스레드가 시끄러워진다 | 메인 창구를 하나로 둔다 | 하비가 먼저 범위와 역할을 선언한다 |
| 좋은 논의가 Slack 안에만 남는다 | 저장 위치를 요청 단계에서 정한다 | GitHub/WikiDocs/shared-memory/Skill 중 어디로 갈지 표시한다 |
| 완료 보고가 길어져 다음 액션이 안 보인다 | 보고 형식을 고정한다 | `완료 / 남음 / 다음` 3줄 안팎으로 끝낸다 |

## Slack 요청을 역할형 에이전트 작업으로 바꾸기

Slack 한 줄 요청은 하비가 먼저 해석한다. 필요한 경우 방울이/뽀동이/하망이/실행형 에이전트에게 나눠 처리하고, 최종 결과는 다시 짧게 묶어 보고한다.
...
이 요청은 “문장 늘리기”가 아니다. 하비나 뽀동이는 아래처럼 나눠야 한다.
...
남음: WikiDocs 동기화 후 공개 화면 부분 확인 필요.
