...
이 글은 전환의 세부 설정을 설명하기 위한 글이 아니다. 실제 운영에서 어떤 혼선이 있었고, 왜 현재 기준점을 다시 세워야 했는지 설명하기 위한 글이다. 중요한 것은 불필요한 세부값이 아니라 “전환할 때 무엇을 분리해서 봐야 하는가”다.
...
전환 과정에서 사용자가 “뽀동이 소울이 약하다”고 느낀 일이 있었다. 표면적으로는 정체성 파일이 있는지 없는지의 문제처럼 보였다. 하지만 실제로 확인해보면 핵심은 파일 개수가 아니라 **현재 Hermes 구조에서 어떤 문서가 정체성을 담당하고, 어떤 문서가 운영 매뉴얼을 담당하는가**였다.

OpenClaw 시절의 뽀동이는 콘텐츠 품질 책임자라는 캐릭터가 선명했다. Hermes로 넘어오면서 기능형 설명은 남았지만, 사용자가 기대하던 햄스터 에너지와 콘텐츠 퀄리티 책임감은 약해져 있었다. 그래서 결론은 “과거 파일을 그대로 복사하자”가 아니었다.
...
OpenClaw에서 Hermes로 넘어온 이야기는 좋은 운영 지식이 될 수 있다. 다만 책이나 팀 문서로 정리할 때는 반드시 기준을 세워야 한다.
...
즉 책에 남길 것은 전환의 이야기와 판단 기준이다. 세부 설정값이나 사적인 운영 정보는 독자에게도 필요 없고, 운영 리스크만 만든다.
