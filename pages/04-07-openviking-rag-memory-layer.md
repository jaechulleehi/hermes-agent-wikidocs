...
5. 보호해야 할 정보나 불필요한 세부값이 섞이지 않았는가?
...
우리 OpenViking 파일럿도 이 기준으로 봤다. 패키지 설치와 파일럿 문서는 준비됐지만, 서버 기동과 provider 설정 검증이 끝나야 실사용 전환이 가능했다. 그래서 책이나 팀 문서로 정리할 때는 “이미 모든 기억 문제가 해결됐다”가 아니라 “외부 회수층을 붙이는 방향으로 기억 시스템을 확장하고 있다”라고 설명하는 것이 정확하다.
...
안 된다. 보호해야 할 정보, 불필요한 세부값, 오래된 문서, source of truth가 아닌 자료를 그대로 넣으면 잘못된 회수가 늘어난다. 넣기 전에 정리와 평가 질문이 필요하다.
